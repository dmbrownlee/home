locals {
  k8s_control_plane_vlan = var.vlans[index(var.vlans.*.comment, "DMZ")]
}
...
resource "proxmox_virtual_environment_vm" "k8s_control_plane" {
  ...
  initialization {
    datastore_id = var.vm_storage
    dns {
      servers = local.k8s_control_plane_vlan.ipv4_dns_servers
    }
    ip_config {
      ipv4 {
        address = each.value.ipv4_address
        gateway = local.k8s_control_plane_vlan.ipv4_gateway
      }
    }
    user_account {
      username = var.ciuser
